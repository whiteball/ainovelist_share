...
<?= $this->section('title') ?> - <?= esc($start_date) ?>～<?= esc($end_date) ?>のランキング<?= $this->endSection() ?>

<?= $this->section('ogp') ?>
<?php $description = esc($start_date, 'attr') . 'から' . esc($end_date, 'attr') . 'までの期間内の行われたnovelファイルのダウンロード/AIのべりすとへ読み込みの合計数のランキングです。' ?>
<meta property="og:image" content="<?= base_url('img/ogp.png') ?>" />
<meta name="twitter:image" content="<?= base_url('img/ogp.png') ?>" />
<meta property="og:type" content="website" />
<meta property="og:title" content="<?= esc($start_date, 'attr')?>～<?= esc($end_date, 'attr')?>のランキング" />
<meta name="twitter:title" content="<?= esc($start_date, 'attr')?>～<?= esc($end_date, 'attr')?>のランキング">
...
<main class="container">
	<h3><?= esc($start_date) ?>～<?= esc($end_date) ?>のランキング</h3>
	<hr>
	<div>
		<?= esc($start_date) ?>から<?= esc($end_date) ?>までの期間内の行われたnovelファイルのダウンロード/AIのべりすとへ読み込みの合計数のランキングです。
	</div>
